...
This linter verifies that cached connector implementations in `internal/forge`
implement the same interfaces as their uncached counterparts.
...
To add a new connector pair to the linter, edit `main.go` and add an entry to
the `connectorPairs` slice:
