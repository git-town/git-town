...
files_with_dashes=$(find . -name '*-*' | grep -v node_modules | grep -v package-lock.json | grep -v vendor | grep -v '.git' | grep -v './website' | grep -v '.tool-versions' | grep -v 'text-runner.jsonc' | grep -v './dprint-changelog.json' | grep -v './.gherkin-*' | grep -v 'tools(/|\\)rta')
