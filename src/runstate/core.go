...
func (runState *RunState) AddPushBranchStepAfterCurrentBranchSteps(repo *git.InternalCommands) error {
	popped := StepList{}
	for {
...
func (runState *RunState) MarkAsUnfinished(repo *git.InternalCommands) error {
	currentBranch, err := repo.CurrentBranch()
	if err != nil {
		return err
	}
