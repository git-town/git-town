...
func determineKillConfig(args []string, run *git.ProdRunner) (*killConfig, error) {
	...
	return &killConfig{
		childBranches:       run.Config.Ancestry.Children(targetBranch),
		...
		targetBranchParent:  run.Config.Ancestry.Parent(targetBranch),
	}, nil
}
...
func killStepList(config *killConfig, run *git.ProdRunner) (runstate.StepList, error) {
	...
	case config.isTargetBranchLocal:
		...
		result.Append(&steps.DeleteParentBranchStep{Branch: config.targetBranch, Parent: run.Config.Ancestry.Parent(config.targetBranch)})
