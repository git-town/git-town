...
func killFeatureBranch(list *steps.List, finalUndoList *steps.List, config killConfig) {
	...
	list.Add(&step.DeleteLocalBranch{Branch: config.targetBranch.LocalName, Parent: config.mainBranch.Location(), Force: false})
...
func removeBranchFromLineage(args removeBranchFromLineageArgs) config.Lineage {
	...
	for _, child := range childBranches {
		...
		args.list.Add(&step.PrintMessage{Message: fmt.Sprintf(messages.BranchParentChanged, child, args.parent)})
