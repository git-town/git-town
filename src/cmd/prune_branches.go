...
func pruneBranchesStepList(config *pruneBranchesConfig, run *git.ProdRunner) (runstate.StepList, error) {
	...
	for _, branchWithDeletedRemote := range config.localBranchesWithDeletedTrackingBranches {
		...
		parent := run.Config.Ancestry.Parent(branchWithDeletedRemote)
		if parent != "" {
			for _, child := range run.Config.Ancestry.Children(branchWithDeletedRemote) {
				result.Append(&steps.SetParentStep{Branch: child, ParentBranch: parent})
			}
			result.Append(&steps.DeleteParentBranchStep{Branch: branchWithDeletedRemote, Parent: run.Config.Ancestry.Parent(branchWithDeletedRemote)})
		}
