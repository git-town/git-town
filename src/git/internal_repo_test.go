...
func TestRepo(t *testing.T) {
	...
	t.Run("BranchAuthors", func(t *testing.T) {
		...
		assert.NoError(t, err)
		authors, err := repo.InternalRepo.BranchAuthors("branch", "initial")
		assert.NoError(t, err)
	...
	t.Run(".CheckoutBranch()", func(t *testing.T) {
		...
		assert.NoError(t, err)
		err = repo.InternalRepo.CheckoutBranch("branch1")
		assert.NoError(t, err)
		currentBranch, err := repo.CurrentBranch()
		assert.NoError(t, err)
		assert.Equal(t, "branch1", currentBranch)
		err = repo.InternalRepo.CheckoutBranch("initial")
		assert.NoError(t, err)
		currentBranch, err = repo.CurrentBranch()
		assert.NoError(t, err)
	...
	t.Run(".CreateFeatureBranch()", func(t *testing.T) {
		t.Parallel()
		repo := test.CreateTestGitTownRepo(t).InternalRepo
	...
	t.Run(".CurrentBranch()", func(t *testing.T) {
		...
		err = repo.CheckoutBranch("initial")
		assert.NoError(t, err)
		branch, err = repo.InternalRepo.CurrentBranch()
		assert.NoError(t, err)
	...
	t.Run(".HasOpenChanges()", func(t *testing.T) {
		...
		assert.NoError(t, err)
		has, err = repo.InternalRepo.HasOpenChanges()
		assert.NoError(t, err)
		assert.True(t, has)
	})
	...
	t.Run(".HasRebaseInProgress()", func(t *testing.T) {
		t.Parallel()
		repo := test.CreateRepo(t).InternalRepo
	...
	t.Run(".HasRemote()", func(t *testing.T) {
		...
		repo, err := origin.Clone(repoDir)
		assert.NoError(t, err)
		has, err := repo.InternalRepo.HasOrigin()
		assert.NoError(t, err)
		assert.True(t, has)
		has, err = repo.InternalRepo.HasRemote("zonk")
		assert.NoError(t, err)
		assert.False(t, has)
	})
	...
	t.Run(".HasTrackingBranch()", func(t *testing.T) {
		...
		assert.NoError(t, err)
		assert.True(t, has)
		has, err = devRepo.InternalRepo.HasTrackingBranch("b2")
		assert.NoError(t, err)
		assert.False(t, has)
		has, err = devRepo.InternalRepo.HasTrackingBranch("b3")
		assert.NoError(t, err)
		assert.False(t, has)
	})
	...
	t.Run(".PreviouslyCheckedOutBranch()", func(t *testing.T) {
		...
		assert.NoError(t, err)
		have, err := repo.InternalRepo.PreviouslyCheckedOutBranch()
		assert.NoError(t, err)
	...
	t.Run(".RemoteBranches()", func(t *testing.T) {
		...
		err = repo.Fetch()
		assert.NoError(t, err)
		branches, err := repo.InternalRepo.RemoteBranches()
		assert.NoError(t, err)
