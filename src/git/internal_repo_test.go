...
func TestRepo(t *testing.T) {
	...
	t.Run(".ConnectTrackingBranch()", func(t *testing.T) {
		...
		assert.NoError(t, err)
		repo := test.NewRepo(repoDir, repoDir, "").InternalRepo
		err = repo.AddRemote(config.OriginRemote, origin.Dir)
		assert.NoError(t, err)
		err = repo.Fetch()
		assert.NoError(t, err)
	...
	t.Run(".HasBranchesOutOfSync()", func(t *testing.T) {
		...
		t.Run("branch is behind", func(t *testing.T) {
			...
			assert.NoError(t, err)
			have, err := env.DevRepo.InternalRepo.HasBranchesOutOfSync()
			assert.NoError(t, err)
			assert.True(t, have)
		})
	})
	...
	t.Run(".LocalAndOriginBranches()", func(t *testing.T) {
		...
		err = repo.Fetch()
		assert.NoError(t, err)
		branches, err := repo.InternalRepo.LocalAndOriginBranches()
		assert.NoError(t, err)
