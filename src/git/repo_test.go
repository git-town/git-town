...
func TestGitRepo_CreateFeatureBranch(t *testing.T) {
	...
	assert.Equal(t, []string{"main"}, repo.Config().GetAncestorBranches("f1"))
}
...
func TestGitRepo_CreateFeatureBranchNoParent(t *testing.T) {
	...
	repo.Config().Reload()
	assert.True(t, repo.Config().IsFeatureBranch("f1"))
	assert.Equal(t, []string(nil), repo.Config().GetAncestorBranches("f1"))
}
...
func TestGitRepo_HasOpenChanges(t *testing.T) {
	repo := test.CreateTestRepo(t)
