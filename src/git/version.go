...
func isVersionRequirementSatisfied() bool {
	...
	logs.FatalOn(err, "Error compiling version regular expression")
	...
	logs.FatalOn(err, "Error convering major version to int")
	minorVersion, err := strconv.Atoi(matches[2])
	logs.FatalOn(err, "Error convering minor version to int")
