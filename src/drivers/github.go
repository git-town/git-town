...
func (driver GithubCodeHostingDriver) MergePullRequest(options MergePullRequestOptions) error {
	// TODO: get pull requests by number which have options.Branch as a base, update them to have parentBranch as a base
	pullRequestNumber, err := driver.getPullRequestNumber(options.Repository, options.Branch, options.ParentBranch)
	if err != nil {
		return err
	}
	query := url.Values{}
...
func (driver GithubCodeHostingDriver) updatePullRequestBase(repository string, branch string, parentBranch string) error {
	return nil
}
