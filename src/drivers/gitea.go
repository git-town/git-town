...
func tryCreateGitea(config giteaConfig) CodeHostingDriver {
	if config.GetCodeHostingDriverName() != "gitea" && config.GetCodeHostingOriginHostname() != "gitea.com" {
		return nil
	}
	originURL := config.GetRemoteOriginURL()
	result := giteaCodeHostingDriver{
		originURL:  originURL,
