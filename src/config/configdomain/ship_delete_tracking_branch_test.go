...
func TestShipDeleteTrackingBranch(t *testing.T) {
	...
	t.Run("ParseShipDeleteTrackingBranch", func(t *testing.T) {
		...
		t.Run("parsable value", func(t *testing.T) {
			t.Parallel()
			have, err := configdomain.ParseShipDeleteTrackingBranch("yes", "unit test")
			must.NoError(t, err)
		...
		t.Run("invalid value", func(t *testing.T) {
			t.Parallel()
			_, err := configdomain.ParseShipDeleteTrackingBranch("zonk", "test")
			must.EqOp(t, `invalid value for test: "zonk". Please provide either "yes" or "no"`, err.Error())
		})
	})
}
