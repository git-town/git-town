...
function add_non_feature_branch {
  ...
  if [ "$(is_non_feature_branch "$branch_name")" == true ]; then
    echo_error_header
    echo_error "'$branch_name' is already a non-feature branch"
    exit_with_error
  else
...
function add_or_remove_non_feature_branches {
  ...
  if [ -z "$branch_name" ]; then
    echo_error_header
    echo_error "branch name required"
    exit_with_error
  else
    if [ "$operation" == "--add" ]; then
      ...
      echo_error_header
      echo_error "unknown option: $operation"
      exit_with_error
    fi
  fi
}
...
function remove_non_feature_branch {
  ...
  if [ "$(is_non_feature_branch "$branch_name")" == true ]; then
    ...
    echo_error_header
    echo_error "'$branch_name' is not a non-feature branch"
    exit_with_error
  fi
}
...
function show_config {
  echo_bold "Main branch:"
  show_main_branch
  echo
  echo_bold "Non-feature branches:"
