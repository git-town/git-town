...
When not set, Git Town uses the
[feature sync strategy](sync-feature-strategy.md).

## options

This setting accepts the same options
as the [feature sync strategy](sync-feature-strategy.md#options).
...
for prototype branches by setting the `GIT_TOWN_SYNC_PROTOTYPE_STRATEGY`
environment variable.
