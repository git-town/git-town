...
If the branch name you provide already includes the configured prefix,
Git Town won't add it again.
For instance, with prefix `kevgo-`,
running `git town hack kevgo-example` creates `kevgo-example`
(not `kevgo-kevgo-example`).

## config file

In the [config file](../configuration-file.md),
define the branch prefix within the `[create]` section:
...
If you want to use your GitHub username as the branch prefix,
set this environment variable once with a call like this:
