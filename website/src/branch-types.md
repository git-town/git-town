...
configured, you can run `git town sync` or `git town sync --all` at any time and each of
...
reason `git town sync` does not pull updates from the parent branch of a contribution
...
[git town contribute](commands/contribute.md) on it. Convert a contribution branch
back to a feature branch by running [git town hack](commands/hack.md) on it.
...
Unlike with contributing branches, `git town sync` does not push your local commits
made to an observed branch to its tracking branch.

You can make any feature branch an observed branch by running
[git town observe](commands/observe.md) on it. Convert an observed branch back to a
feature branch by running [git town hack](commands/hack.md) on it.

## Parked Branches

Parked branches don't get synced at all unless you run `git town sync` directly on a
...
You can park any feature branch by running [git town park](commands/park.md) on it.
...
[git town prototype](commands/prototype.md) on it. To convert a prototype branch to a
feature branch, use [git town hack](commands/hack.md).
