...
cukethiswin:  # runs the end-to-end tests that have a @this tag on Windows
	go install -ldflags "-X github.com/git-town/git-town/v14/pkg/cmd.version=-dev -X github.com/git-town/git-town/v14/pkg/cmd.buildDate=1/2/3"
...
lint: tools/rta@${RTA_VERSION}  # lints the main codebase concurrently
	...
	@tools/rta --available alphavet && go vet "-vettool=$(shell tools/rta --which alphavet)" $(shell go list ./... | grep -v src/cmd)
...
# @tools/rta ireturn --reject="github.com/git-town/git-town/v14/pkg/prelude.Option" github.com/git-town/git-town/...
