...
func TestRunner(t *testing.T) {
	...
	t.Run(".HasBranchesOutOfSync()", func(t *testing.T) {
		t.Run("branches are in sync", func(t *testing.T) {
			...
			assert.NoError(t, err)
			err = commands.PushBranchToRemote(&runner, "branch1", config.OriginRemote)
			assert.NoError(t, err)
			have, err := commands.HasUnsyncedBranches(&runner)
			assert.NoError(t, err)
			assert.False(t, have)
		})
		...
		t.Run("branch is ahead", func(t *testing.T) {
			...
			assert.NoError(t, err)
			have, err := commands.HasUnsyncedBranches(&env.DevRepo)
			assert.NoError(t, err)
			assert.True(t, have)
		})
