...
func NewStandardFixture(dir string) (Fixture, error) {
	...
	gitEnv.DevRepo, err = Clone(&originRepo, gitEnv.developerRepoPath())
	if err != nil {
...
func (env *Fixture) AddUpstream() error {
	repo, err := Clone(&env.DevRepo, filepath.Join(env.Dir, "upstream"))
	if err != nil {
...
func (env *Fixture) AddCoworkerRepo() error {
	coworkerRepo, err := Clone(env.OriginRepo, env.coworkerRepoPath())
	if err != nil {
