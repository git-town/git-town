...
func CoworkerSteps(suite *godog.Suite, fs *FeatureState) {
	suite.Step(`^I am collaborating with a coworker$`, func() error {
		return fs.state.gitEnvironment.AddCoworkerRepo()
	})

	suite.Step(`^my coworker fetches updates$`, func() error {
		return fs.state.gitEnvironment.CoworkerRepo.Fetch()
	})

	suite.Step(`^my coworker sets the parent branch of "([^"]*)" as "([^"]*)"$`, func(childBranch, parentBranch string) error {
		_ = fs.state.gitEnvironment.CoworkerRepo.Configuration(false).SetParentBranch(childBranch, parentBranch)
		return nil
	})

	suite.Step(`^my coworker is on the "([^"]*)" branch$`, func(branchName string) error {
		return fs.state.gitEnvironment.CoworkerRepo.CheckoutBranch(branchName)
	})

	suite.Step(`^my coworker runs "([^"]+)"$`, func(command string) error {
		fs.state.lastRunResult, fs.state.lastRunErr = fs.state.gitEnvironment.CoworkerRepo.Shell.RunString(command)
		return nil
	})
}
