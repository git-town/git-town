...
func PrintSteps(suite *godog.Suite, fs *FeatureState) {
	suite.Step(`^it does not print "([^\"]*)"$`, func(text string) error {
		if strings.Contains(fs.state.lastRunResult.OutputSanitized(), text) {
	...
	suite.Step(`^it prints:$`, func(expected *messages.PickleStepArgument_PickleDocString) error {
		if !strings.Contains(fs.state.lastRunResult.OutputSanitized(), expected.Content) {
			return fmt.Errorf("text not found:\n\nEXPECTED: %q\n\nACTUAL:\n\n%q", expected.Content, fs.state.lastRunResult.OutputSanitized())
		}
		return nil
	})

	suite.Step(`^it prints no output$`, func() error {
		output := fs.state.lastRunResult.OutputSanitized()
		if output != "" {
	...
	suite.Step(`^it prints the error:$`, func(expected *messages.PickleStepArgument_PickleDocString) error {
		if !strings.Contains(fs.state.lastRunResult.OutputSanitized(), expected.Content) {
			return fmt.Errorf("text not found: %s\n\nactual text:\n%s", expected.Content, fs.state.lastRunResult.OutputSanitized())
		}
		if fs.state.lastRunErr == nil {
	...
	suite.Step(`^it prints the initial configuration prompt$`, func() error {
		...
		if !fs.state.lastRunResult.OutputContainsText(expected) {
			return fmt.Errorf("text not found:\n\nEXPECTED: %q\n\nACTUAL:\n\n%q\n----------------------------", expected, fs.state.lastRunResult.Output())
		}
		return nil
	})
	...
	suite.Step(`^I am not prompted for any parent branches$`, func() error {
		...
		if fs.state.lastRunResult.OutputContainsText(notExpected) {
			return fmt.Errorf("text found:\n\nDID NOT EXPECT: %q\n\nACTUAL\n\n%q\n----------------------------", notExpected, fs.state.lastRunResult.Output())
		}
		return nil
	})
}
