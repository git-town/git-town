...
func TagSteps(suite *godog.Suite, fs *FeatureState) {
	...
	suite.Step(`^my repo has a remote tag "([^"]+)" that is not on a branch$`, func(name string) error {
		return fs.state.gitEnvironment.OriginRepo.CreateStandaloneTag(name)
	})
}
