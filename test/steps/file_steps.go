...
func FileSteps(suite *godog.Suite, fs *FeatureState) {
	suite.Step(`^I don't have any uncommitted files$`, func() error {
		files, err := fs.gitEnv.DevRepo.UncommittedFiles()
		if err != nil {
	...
	suite.Step(`^my uncommitted file is stashed$`, func() error {
		uncommittedFiles, err := fs.gitEnv.DevRepo.UncommittedFiles()
		if err != nil {
			return err
		}
		for ucf := range uncommittedFiles {
			if uncommittedFiles[ucf] == fs.uncommittedFileName {
				return fmt.Errorf("expected file %q to be stashed but it is still uncommitted", fs.uncommittedFileName)
			}
		}
		stashSize, err := fs.gitEnv.DevRepo.StashSize()
		if err != nil {
			return err
		}
		if stashSize != 1 {
	...
	suite.Step(`^my workspace still contains the file "([^"]*)" with content "([^"]*)"$`, func(file, expectedContent string) error {
		actualContent, err := fs.gitEnv.DevRepo.FileContent(file)
		if err != nil {
			return err
		}
	...
	suite.Step(`^my repository (?:now|still) has the following committed files$`, func(table *messages.PickleStepArgument_PickleTable) error {
		fileTable, err := fs.gitEnv.DevRepo.FilesInBranches()
		if err != nil {
