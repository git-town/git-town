...
// Repo provides Git functionality for unit and end-to-end tests.
type Repo struct {
	testshell.Mocking
	git.RepoConfig
	git.BackendCommands
...
func Initialize(workingDir, homeDir, binDir string) (Repo, error) {
...
func New(workingDir, homeDir, binDir string) Repo {
...
func CreateGitTown(t *testing.T) Repo {
	t.Helper()
	repo := Create(t)
	err := CreateBranch(&repo, "main", "initial")
	assert.NoError(t, err)
...
func Clone(original *Repo, targetDir string) (Repo, error) {
	_, err := original.Run("git", "clone", original.WorkingDir, targetDir)
	if err != nil {
		return Repo{}, fmt.Errorf("cannot clone repo %q: %w", original.WorkingDir, err)
	}
	return New(targetDir, original.HomeDir, original.BinDir), nil
}

func (r *Repo) ProdGit() *git.BackendCommands {
	return &r.BackendCommands
}

func (r *Repo) Config() *git.RepoConfig {
	return &r.RepoConfig
}
