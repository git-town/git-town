...
func TestTestCommands(t *testing.T) {
	...
	t.Run("AddRemote", func(t *testing.T) {
		...
		dev.AddRemote(git.REMOTE_ORIGIN, origin.WorkingDir)
		remotes, err = dev.Remotes(dev)
		must.NoError(t, err)
		must.Eq(t, gitdomain.Remotes{git.REMOTE_ORIGIN}, remotes)
	})
	...
	t.Run("ConnectTrackingBranch", func(t *testing.T) {
		...
		runtime.AddRemote(git.REMOTE_ORIGIN, origin.WorkingDir)
		runtime.Fetch()
	...
	t.Run("Fetch", func(t *testing.T) {
		...
		origin := testruntime.Create(t)
		repo.AddRemote(git.REMOTE_ORIGIN, origin.WorkingDir)
		repo.Fetch()
	})
	...
	t.Run("HasBranchesOutOfSync", func(t *testing.T) {
		t.Run("branches are in sync", func(t *testing.T) {
			...
			devRepo.PushBranchToRemote(gitdomain.NewLocalBranchName("branch1"), git.REMOTE_ORIGIN)
	...
	t.Run("PushBranchToRemote", func(t *testing.T) {
		...
		dev.PushBranchToRemote(gitdomain.NewLocalBranchName("b1"), git.REMOTE_ORIGIN)
	...
	t.Run("RemoveRemote", func(t *testing.T) {
		...
		origin := testruntime.Create(t)
		repo.AddRemote(git.REMOTE_ORIGIN, origin.WorkingDir)
		repo.RemoveRemote(git.REMOTE_ORIGIN)
