...
func Steps(suite *godog.Suite, state *ScenarioState) {
	...
	suite.Step(`^no branch hierarchy exists now$`, func() error {
		state.gitEnv.DevRepo.Config.Store.Reload()
	...
	suite.Step(`^setting "new-branch-push-flag" is "(true|false)"$`, func(value string) error {
		_, err := state.gitEnv.DevRepo.Config.Store.SetLocalConfigValue("git-town.new-branch-push-flag", value)
		return err
	})

	suite.Step(`^setting "new-branch-push-flag" is globally "(true|false)"$`, func(value string) error {
		_, err := state.gitEnv.DevRepo.Config.Store.SetGlobalConfigValue("git-town.new-branch-push-flag", value)
		return err
	})
	...
	suite.Step(`^setting "push-new-branches" is (globally )?"([^"]*)"$`, func(global string, value string) error {
		...
		_, err = state.gitEnv.DevRepo.Config.Store.SetLocalConfigValue("git-town.push-new-branches", value)
		return err
	})
	...
	suite.Step(`^setting "offline" is "([^"]*)"$`, func(value string) error {
		_, err := state.gitEnv.DevRepo.Config.Store.SetGlobalConfigValue("git-town.offline", value)
		return err
	})
	...
	suite.Step(`^(local )?setting "push-hook" is "(.*)"$`, func(local, value string) error {
		_, err := state.gitEnv.DevRepo.Config.Store.SetLocalConfigValue("git-town.push-hook", value)
		return err
	})

	suite.Step(`^global setting "push-hook" is "(.*)"$`, func(value string) error {
		_, err := state.gitEnv.DevRepo.Config.Store.SetGlobalConfigValue("git-town.push-hook", value)
		return err
	})

	suite.Step(`^local setting "push-hook" is now "(.*)"$`, func(want string) error {
		state.gitEnv.DevRepo.Config.Store.Reload()
		have := state.gitEnv.DevRepo.Config.Store.LocalConfigValue("git-town.push-hook")
		if have != want {
	...
	suite.Step(`^global setting "push-hook" is now "(.*)"$`, func(want string) error {
		state.gitEnv.DevRepo.Config.Store.Reload()
		have := state.gitEnv.DevRepo.Config.Store.GlobalConfigValue("git-town.push-hook")
		if have != want {
