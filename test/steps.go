...
func Steps(suite *godog.Suite, state *ScenarioState) {
	...
	suite.Step(`^a feature branch "([^"]+)" as a child of "([^"]+)"$`, func(branch, parentBranch string) error {
		...
		return state.gitEnv.DevRepo.PushBranch(git.PushArgs{BranchName: branch, Remote: "origin"})
	})
	...
	suite.Step(`^the current branch is a (local )?(feature|perennial) branch "([^"]*)"$`, func(localStr, branchType, branch string) error {
		...
		if !isLocal {
			state.initialRemoteBranches = append(state.initialRemoteBranches, branch)
			err := state.gitEnv.DevRepo.PushBranch(git.PushArgs{BranchName: branch, Remote: "origin"})
			if err != nil {
				return err
			}
		}
	...
	suite.Step(`^the (local )?perennial branches "([^"]+)" and "([^"]+)"$`, func(localStr, branch1, branch2 string) error {
		...
		if !isLocal {
			...
			err = state.gitEnv.DevRepo.PushBranch(git.PushArgs{BranchName: branch1, Remote: "origin"})
			if err != nil {
				return err
			}
			return state.gitEnv.DevRepo.PushBranch(git.PushArgs{BranchName: branch2, Remote: "origin"})
		}
		return nil
	})
