...
Then(/^I see a browser open to a new (.+?) pull request for the "(.+?)" branch$/) do |domain, branch_name|
  expect(@last_run_result.out).to eql "#{@tool} called with: #{pull_request_url domain, branch_name}\n"
end


Then(/^I see a browser open to the homepage of my (.+?) repository$/) do |domain|
