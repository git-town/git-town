Feature: git prune-branches: don't remove the current empty feature branch if there are open changes
  ...
  Background:
    ...
    And I have a stale feature branch named "stale-feature-1" with its tip at "Initial commit"
    And I have a stale feature branch named "stale-feature-2" with its tip at "Initial commit"
  ...
  Scenario: undoing the prune
    ...
    Then it runs the commands
      ...
      |                 | git checkout main                                      |
      | main            | git branch stale-feature-2 <%= sha 'Initial commit' %> |
