Feature: rebase a branch that contains amended commits
  ...
  Background:
    ...
    And the commits
      ...
      | feature-1 | local, origin | commit 1 | file_1    | one          |
    ...
    And I amend this commit
      | BRANCH    | LOCATION | MESSAGE  | FILE NAME | FILE CONTENT |
      | feature-1 | local    | commit 1 | file_1    | another one  |
    And the current branch is "feature-2"
  ...
  Scenario: result
    Then Git Town runs the commands
      ...
      | feature-2 | git rebase --onto feature-1 {{ sha-before-run 'commit 1' }} --no-update-refs |
      |           | git push --force-with-lease --force-if-includes                              |
      |           | git rebase --onto feature-1 {{ sha-before-run 'commit 1' }} --no-update-refs |
  ...
  Scenario: undo
    ...
    Then Git Town runs the commands
      ...
      |           | git push --force-with-lease --force-if-includes                                        |
      |           | git push --force-with-lease origin {{ sha-in-origin-before-run 'commit 1' }}:feature-1 |
    And these commits exist now
      | BRANCH    | LOCATION      | MESSAGE  |
      | feature-1 | local         | commit 1 |
      |           | origin        | commit 1 |
      | feature-2 | local         | commit 1 |
