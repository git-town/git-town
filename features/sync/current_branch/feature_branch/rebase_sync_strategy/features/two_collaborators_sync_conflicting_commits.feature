Feature: collaborative feature branch syncing
  ...
  Scenario: I and my coworker work together on a branch
    ...
    And the commits
      ...
      | feature | local    | my commit       | my_file.txt  | my content       |
      |         | origin   | coworker commit | cow_file.txt | coworker content |
    ...
    And these commits exist now
      | BRANCH  | LOCATION         | MESSAGE         | FILE NAME    | FILE CONTENT     |
      | feature | local,origin     | my commit       | my_file.txt  | my content       |
      |         | coworker, origin | coworker commit | cow_file.txt | coworker content |
    ...
    Then it runs the commands
      ...
      |         | git rebase main             |
      |         | git push --force-with-lease |
    And the coworker adds this commit:
      | BRANCH  | feature            |
      | FILE    | file.txt           |
      | CONTENT | coworker content 1 |
    And all branches are now synchronized
    And these commits exist now
