...
Feature: beam from a branch without parent
  ...
  Scenario: result
    Then Git Town runs the commands
      ...
      | branch-2 | git cherry-pick {{ sha 'branch commit 1' }}                                                           |
      |          | git checkout branch-1                                                                                 |
      | branch-1 | git -c rebase.updateRefs=false rebase --onto {{ sha 'branch commit 1' }}^ {{ sha 'branch commit 1' }} |
    ...
    And these commits exist now
      | BRANCH   | LOCATION | MESSAGE         |
      | branch-1 | local    | branch commit 2 |
      | branch-2 | local    | branch commit 1 |
  ...
  Scenario: undo
    ...
    Then Git Town runs the commands
      ...
      | branch-1 | git reset --hard {{ sha 'branch commit 2' }} |
