...
func determineSharedShipData(args determineSharedShipDataArgs) (sharedShipData, configdomain.ProgramFlow, error) {
	...
	connector, detectedForgeType, err := args.repo.NewConnector(config.DevURL(args.repo.Backend))
	if err != nil {
		return emptyResult, configdomain.ProgramFlowExit, err
	}
	...
	if len(args.args) > 0 {
		branchToShip = gitdomain.LocalBranchNameOrPanic(stringss.Trim(args.args[0]))
