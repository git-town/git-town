...
func executeSwitch(allBranches configdomain.AllBranches, verbose configdomain.Verbose, merge configdomain.SwitchUsingMerge, branchTypes []configdomain.BranchType) error {
	...
	branchToCheckout, exit, err := dialog.SwitchBranch(branchTypes, branchesAndTypes, data.initialBranch, data.config.Config.Lineage, data.branchesSnapshot.Branches, allBranches, defaultBranchType, data.uncommittedChanges, data.dialogInputs.Next())
	if err != nil || exit {
		return err
	}
