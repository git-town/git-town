	...
	if localBranchToKill, hasLocalBranchToKill := data.branchToKillInfo.LocalName.Get(); hasLocalBranchToKill {
		if data.initialBranch == localBranchToKill {
			if data.hasOpenChanges {
				...
				prog.Value.Add(&opcodes.Commit{Message: gitdomain.NewCommitMessageOpt("Committing WIP for git town undo"), UseDefaultMessage: git.MissingCommitMessageUseDefault})
