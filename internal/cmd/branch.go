...
func determineBranchData(repo execute.OpenRepoResult, verbose configdomain.Verbose) (data branchData, exit bool, err error) {
	...
	defaultBranchType := repo.NormalConfig.Config.Value.DefaultBranchType
	colors := colors.NewDialogColors()
	branchesAndTypes := repo.UnvalidatedConfig.Config.Value.UnvalidatedBranchesAndTypes(branchesSnapshot.Branches.Names(), repo.NormalConfig.Config.Value)
	return branchData{
		...
		lineage:           repo.NormalConfig.Config.Value.Lineage,
	}, false, err
}
