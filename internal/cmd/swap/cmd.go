...
func determineSwapData(repo execute.OpenRepoResult) (data swapData, flow configdomain.ProgramFlow, err error) {
	...
	case configdomain.ProgramFlowExit, configdomain.ProgramFlowRestart:
		return data, configdomain.ProgramFlowExit, nil
	}
	if branchesSnapshot.DetachedHead {
		return data, flow, errors.New(messages.SwapRepoHasDetachedHead)
	}
	...
	if !hasBranchToSwapInfo {
		return data, flow, fmt.Errorf(messages.BranchDoesntExist, currentBranch)
	}
	if currentBranchInfo.SyncStatus == gitdomain.SyncStatusOtherWorktree {
		return data, flow, fmt.Errorf(messages.BranchOtherWorktree, currentBranch)
	}
	...
	if !hasParentBranch {
		return data, flow, errors.New(messages.SwapNoParent)
	}
	parentBranchInfo, hasParentBranchInfo := branchesSnapshot.Branches.FindByLocalName(parentBranch).Get()
	if !hasParentBranchInfo {
		return data, flow, fmt.Errorf(messages.SwapParentNotLocal, parentBranch)
	}
	...
	if !hasGrandParentBranch {
		return data, flow, errors.New(messages.SwapNoGrandParent)
	}
	...
	if branchContainsMerges {
		return data, flow, fmt.Errorf(messages.SwapNeedsCompress, currentBranch)
	}
	...
	if parentContainsMerges {
		return data, flow, fmt.Errorf(messages.SwapNeedsCompress, parentBranch)
	}
