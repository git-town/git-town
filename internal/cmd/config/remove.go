...
func executeRemoveConfig(verbose configdomain.Verbose) error {
	...
	err = repo.UnvalidatedConfig.GitConfig.RemoveLocalGitConfiguration(repo.NormalConfig.Config.Value.Lineage)
	if err != nil {
		return err
	}
	aliasNames := maps.Keys(repo.NormalConfig.Config.Value.Aliases)
	slices.Sort(aliasNames)
	for _, aliasName := range aliasNames {
		if strings.HasPrefix(repo.NormalConfig.Config.Value.Aliases[aliasName], "town ") {
