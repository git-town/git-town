...
func printConfig(config config.UnvalidatedConfig, redact configdomain.Redact) {
	...
	fmt.Println()
	print.Header("Diff")
	print.Entry("name only", format.Bool(config.NormalConfig.NameOnly.DisplayNameOnly()))
	fmt.Println()
	...
	print.Entry("breadcrumb style", format.StringsSetting(config.NormalConfig.ProposalBreadcrumbStyle.String()))
	fmt.Println()
	print.Header("Ship")
