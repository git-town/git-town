...
func TestNewlineDelineated(t *testing.T) {
	...
	t.Run("Sanitize", func(t *testing.T) {
		...
		t.Run("empty input", func(t *testing.T) {
			t.Parallel()
			give := bytestream.NewlineDelineated([]byte{})
			have := give.Sanitize()
			want := bytestream.Sanitized([]byte{})
			must.SliceEqOp(t, want, have)
		})
		t.Run("no secrets", func(t *testing.T) {
			t.Parallel()
			give := bytestream.NewlineDelineated([]byte("hello world"))
			have := give.Sanitize()
			want := bytestream.Sanitized([]byte("hello world"))
			must.SliceEqOp(t, want, have)
		})
