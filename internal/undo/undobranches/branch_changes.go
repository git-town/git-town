...
<<<<<<< HEAD:src/undo/undobranches/branch_changes.go
...
>>>>>>> main:internal/undo/undobranches/branch_changes.go
)
...
func (self BranchChanges) UndoProgram(args BranchChangesUndoProgramArgs) program.Program {
	...
	for _, inconsistentChange := range inconsistentChangedFeatures {
		...
		if hasBeforeLocal && hasBeforeRemote && hasAfterSHAs {
			...
			doPush, abort, err := dialog.UndoForcePush(beforeRemoteName, afterLocalSHA, beforeLocalSHA, args.Inputs.Next())
			if err != nil {
				panic(err)
			}
			if abort {
	...
	for _, addedRemoteBranch := range self.RemoteAdded {
		if addedRemoteBranch.Remote() != gitdomain.RemoteUpstream {
			doPush, abort, err := dialog.UndoCreateRemoteBranch(addedRemoteBranch, args.Inputs.Next())
			if err != nil {
				panic(err)
			}
			if abort {
