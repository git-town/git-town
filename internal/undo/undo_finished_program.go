...
func CreateUndoForFinishedProgram(args CreateUndoProgramArgs) program.Program {
	...
	if !args.RunState.IsFinished() && args.HasOpenChanges {
		...
		result.Value.Add(&opcodes.Commit{Message: gitdomain.NewCommitMessageOpt("Committing WIP for git town undo"), UseDefaultMessage: git.MissingCommitMessageUseDefault})
	}
