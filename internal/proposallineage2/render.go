...
func Render(tree TreeNodeWithProposal, currentBranch gitdomain.LocalBranchName) string {
	...
	builder.WriteString("\n-------------------------\n")
	foundCurrent := false
	renderNode(&builder, tree, currentBranch, 0, &foundCurrent, false)
...
func renderNode(builder *strings.Builder, node TreeNodeWithProposal, currentBranch gitdomain.LocalBranchName, depth int, foundCurrent *bool, useTabIndent bool) {
	if useTabIndent {
	...
	if *foundCurrent {
		...
		if isCurrentBranch {
			// Determine indentation type based on whether current node has a proposal
			_, currentHasProposal := node.Proposal.Get()
			childUseTabIndent := !currentHasProposal
			for _, child := range node.Children {
				if _, hasProposal := child.Proposal.Get(); hasProposal {
					renderNode(builder, child, currentBranch, depth+1, foundCurrent, childUseTabIndent)
				}
			}
		}
		...
		for _, child := range node.Children {
			renderNode(builder, child, currentBranch, depth+1, foundCurrent, useTabIndent)
		}
	}
}
