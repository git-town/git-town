...
func (self *Commands) LoadSnapshot(runner subshelldomain.RunnerQuerier, scopeOpt Option[configdomain.ConfigScope], updateOutdated configdomain.UpdateOutdatedSettings) (configdomain.SingleSnapshot, error) {
	...
	for _, line := range strings.Split(output, "\x00") {
		...
		if updateOutdated.IsTrue() && hasScope {
			...
			if keyIsDeprecated {
				self.UpdateDeprecatedSetting(runner, scope, configKey, newKey, value)
				configKey = newKey
			}
			if configKey != configdomain.KeyPerennialBranches && value == "" {
				_ = self.RemoveConfigValue(runner, scope, configKey)
				continue
			}
			...
			for _, update := range configdomain.ConfigUpdates {
				if configKey == update.Before.Key && value == update.Before.Value {
					self.UpdateDeprecatedSetting(runner, scope, configKey, update.After.Key, update.After.Value)
			...
			for branchList, branchType := range configdomain.ObsoleteBranchLists {
				if configKey == branchList {
					for _, branch := range strings.Split(value, " ") {
						...
						_ = self.SetConfigValue(runner, scope, branchTypeKey, branchType.String())
					}
					_ = self.RemoveConfigValue(runner, scope, configKey)
...
func (self *Commands) RemoveConfigValue(runner subshelldomain.Runner, scope configdomain.ConfigScope, key configdomain.Key) error {
	args := []string{"config"}
...
func (self *Commands) SetConfigValue(runner subshelldomain.Runner, scope configdomain.ConfigScope, key configdomain.Key, value string) error {
	args := []string{"config"}
	if scope == configdomain.ConfigScopeGlobal {
		args = append(args, "--global")
	}
...
func (self *Commands) UpdateDeprecatedSetting(runner subshelldomain.RunnerQuerier, scope configdomain.ConfigScope, oldKey, newKey configdomain.Key, value string) {
	...
	if err := self.RemoveConfigValue(runner, scope, oldKey); err != nil {
		fmt.Printf(messages.SettingCannotRemove, scope, oldKey, err)
	}
	if err := self.SetConfigValue(runner, scope, newKey, value); err != nil {
