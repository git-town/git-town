...
func NewBranchTypeOverridesInSnapshot(snapshot configdomain.SingleSnapshot, ignoreUnknown bool, runner subshelldomain.Runner) (configdomain.BranchTypeOverrides, error) {
	...
	for key, value := range snapshot { // okay to iterate the map in random order because we assign to a new map
		...
		branchTypeOpt, err := configdomain.ParseBranchTypeOpt(value, key.String())
		if err != nil {
			if ignoreUnknown {
...
func NewPartialConfigFromSnapshot(snapshot configdomain.SingleSnapshot, updateOutdated bool, ignoreUnknown bool, runner subshelldomain.Runner) (configdomain.PartialConfig, error) {
	autoResolve, errAutoResolve := load(snapshot, configdomain.KeyAutoResolve, gohacks.ParseBool[configdomain.AutoResolve], ignoreUnknown)
	autoSync, errAutoSync := load(snapshot, configdomain.KeyAutoSync, gohacks.ParseBool[configdomain.AutoSync], ignoreUnknown)
	...
	detached, errDetached := load(snapshot, configdomain.KeyDetached, gohacks.ParseBool[configdomain.Detached], ignoreUnknown)
	...
	branchType, errNewBranchType := load(snapshot, configdomain.KeyNewBranchType, configdomain.ParseBranchType, ignoreUnknown)
	...
	offline, errOffline := load(snapshot, configdomain.KeyOffline, gohacks.ParseBool[configdomain.Offline], ignoreUnknown)
	...
	pushBranches, errPushBranches := load(snapshot, configdomain.KeyPushBranches, gohacks.ParseBool[configdomain.PushBranches], ignoreUnknown)
	pushHook, errPushHook := load(snapshot, configdomain.KeyPushHook, gohacks.ParseBool[configdomain.PushHook], ignoreUnknown)
	...
	shipDeleteTrackingBranch, errShipDeleteTrackingBranch := load(snapshot, configdomain.KeyShipDeleteTrackingBranch, gohacks.ParseBool[configdomain.ShipDeleteTrackingBranch], ignoreUnknown)
	shipStrategy, errShipStrategy := load(snapshot, configdomain.KeyShipStrategy, configdomain.ParseShipStrategy, ignoreUnknown)
	stash, errStash := load(snapshot, configdomain.KeyStash, gohacks.ParseBool[configdomain.Stash], ignoreUnknown)
...
func load[T any](snapshot configdomain.SingleSnapshot, key configdomain.Key, parseFunc func(string, string) (T, error), ignoreUnknown bool) (Option[T], error) { //nolint:ireturn
