...
func NewPartialConfigFromSnapshot(snapshot SingleSnapshot, updateOutdated bool, removeLocalConfigValue removeLocalConfigValueFunc) (PartialConfig, error) {
	...
	contributionRegex := C(ec, P(ParseContributionRegex(snapshot[KeyContributionRegex])))
	defaultBranchType := C(ec, P(ParseBranchType(snapshot[KeyDefaultBranchType])))
	featureRegex := C(ec, P(ParseFeatureRegex(snapshot[KeyFeatureRegex])))
