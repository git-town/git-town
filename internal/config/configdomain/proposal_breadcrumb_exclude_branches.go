...
func ParseProposalBreadcrumbExcludeBranches(text string, source string) (Option[ProposalBreadcrumbExcludeBranches], error) {
...
func ParseProposalBreadcrumbExcludeBranchesList(texts []string, source string) (Option[ProposalBreadcrumbExcludeBranches], error) {
	...
	for _, text := range texts {
		branchTypeText := strings.TrimSpace(text)
		if branchTypeText == "" {
			continue
		}
