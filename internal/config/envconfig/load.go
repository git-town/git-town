...
func Load(env EnvVars) (configdomain.PartialConfig, error) {
	...
	order, errOrder := configdomain.ParseOrderOpt(env.Get(order), order)
	...
	proposalsShowLineage, errProposalsShowLineage := load(env, proposalsShowLineage, forgedomain.ParseProposalsShowLineageOpt)
