...
func Load(env EnvVars) (configdomain.PartialConfig, error) {
	...
	gitUserEmail := NewOption(gitdomain.GitUserEmail(env.Get(gitAuthorEmail, gitCommitterEmail)))
	gitUserName := NewOption(gitdomain.GitUserName(env.Get(gitAuthorName, gitCommitterName)))
