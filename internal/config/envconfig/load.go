...
func Load(env EnvVars) (configdomain.PartialConfig, error) {
	...
	githubConnectorType, errGitHubConnectorType := forgedomain.ParseGitHubConnectorType(env.Get(githubConnectorType), githubConnectorType)
	gitlabConnectorType, errGitLabConnectorType := forgedomain.ParseGitLabConnectorType(env.Get(gitlabConnectorType), gitlabConnectorType)
	newBranchType, errNewBranchType := configdomain.ParseBranchType(env.Get(newBranchType), newBranchType)
	observedRegex, errObservedRegex := configdomain.ParseObservedRegex(env.Get(observedRegex), observedRegex)
	...
	perennialRegex, errPerennialRegex := configdomain.ParsePerennialRegex(env.Get(perennialRegex), perennialRegex)
	proposalsShowLineage, errProposalsShowLineage := forgedomain.ParseProposalsShowLineage(env.Get(proposalsShowLineage), proposalsShowLineage)
	...
	shipStrategy, errShipStrategy := configdomain.ParseShipStrategy(env.Get(shipStrategy), shipStrategy)
