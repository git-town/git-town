	...
	if !hasMain {
		...
		if err != nil {
			if cannotDisplayDialogs, ok := errors.AsType[*configdomain.CannotDisplayDialogsError](err); ok {
				return config.EmptyValidatedConfig(), false, cannotDisplayDialogs //lint:ignore ST1005 This error contains user-visible guidance, and therefore needs to end with a period.
			}
