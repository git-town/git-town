...
func TestProposalStackLineageBuilder_CheckLineageAndProposals(t *testing.T) {
	...
	tree, err := proposallineage.NewProposalStackLineageTree(args)
	must.NoError(t, err)
...
`[1:]
	...
	// act
	have := proposallineage.NewProposalStackLineageBuilder(args, MutableSome(tree))
...
<sup>[Stack](https://www.git-town.com/how-to/github-actions-breadcrumb.html) generated by [Git Town](https://github.com/git-town/git-town)</sup>
`
		have := proposallineage.NewProposalStackLineageBuilder(args, MutableNone[proposallineage.ProposalStackLineageTree]())
		builder, hasBuilder := have.Get()
		must.True(t, hasBuilder)
		// actual
		actual := builder.Build(args)
...
func TestProposalStackLineageBuilder_NoLineage(t *testing.T) {
	...
	expected := None[proposallineage.ProposalStackLineageBuilder]()

	// act
	actual := proposallineage.NewProposalStackLineageBuilder(args, MutableNone[proposallineage.ProposalStackLineageTree]())
...
func TestProposalStackLineageBuilder_PrependBranch(t *testing.T) {
	...
	*/
	...
	tree, err := proposallineage.NewProposalStackLineageTree(args)
	must.NoError(t, err)
	builder, hasBuilder := proposallineage.NewProposalStackLineageBuilder(args, MutableSome(tree)).Get()
	must.True(t, hasBuilder)
	original := `
