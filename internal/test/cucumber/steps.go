...
`[1:])
	...
	sc.Step(`^branch "([^"]+)" is active in another worktree$`, func(ctx context.Context, branch string) {
		state := ctx.Value(keyScenarioState).(*ScenarioState)
		state.fixture.AddSecondWorktree(gitdomain.LocalBranchNameOrPanic(stringss.TrimSpace(branch)))
	})

	sc.Step(`^branch "([^"]+)" (?:now|still) has type "(\w+)"$`, func(ctx context.Context, branchName, branchTypeName string) error {
		state := ctx.Value(keyScenarioState).(*ScenarioState)
		devRepo := state.fixture.DevRepo.GetOrPanic()
		branch := gitdomain.LocalBranchNameOrPanic(stringss.TrimSpace(branchName))
		wantOpt := asserts.NoError1(configdomain.ParseBranchType(stringss.TrimSpace(branchTypeName), "test"))
	...
	sc.Step(`^commit "([^"]+)" on branch "([^"]+)" now has this full commit message$`, func(ctx context.Context, title, branchText string, expected *godog.DocString) error {
		state := ctx.Value(keyScenarioState).(*ScenarioState)
		devRepo := state.fixture.DevRepo.GetOrPanic()
		branch := gitdomain.LocalBranchNameOrPanic(stringss.TrimSpace(branchText))
	...
	sc.Step(`^Git Town parent setting for branch "([^"]*)" is "([^"]*)"$`, func(ctx context.Context, branch, parent string) error {
		state := ctx.Value(keyScenarioState).(*ScenarioState)
		devRepo := state.fixture.DevRepo.GetOrPanic()
		branchName := gitdomain.LocalBranchNameOrPanic(stringss.TrimSpace(branch))
		parentName := gitdomain.LocalBranchNameOrPanic(stringss.TrimSpace(parent))
	...
	sc.Step(`^I add commit "([^"]*)" to the "([^"]*)" branch$`, func(ctx context.Context, message, branch string) {
		...
		devRepo.CreateCommit(testgit.Commit{
			Branch:   gitdomain.LocalBranchNameOrPanic(stringss.TrimSpace(branch)),
			FileName: "new_file",
	...
	sc.Step(`^in a separate terminal I create branch "([^"]+)" with commits$`, func(ctx context.Context, branchName string, table *godog.Table) {
		...
		newBranch := gitdomain.LocalBranchNameOrPanic(stringss.TrimSpace(branchName))
	...
	sc.Step(`^origin deletes the "([^"]*)" branch$`, func(ctx context.Context, branch string) {
		state := ctx.Value(keyScenarioState).(*ScenarioState)
		state.fixture.OriginRepo.GetOrPanic().RemoveBranch(gitdomain.LocalBranchNameOrPanic(stringss.TrimSpace(branch)))
	})

	sc.Step(`^origin ships the "([^"]*)" branch using the "squash-merge" ship-strategy$`, func(ctx context.Context, branchName string) error {
		state := ctx.Value(keyScenarioState).(*ScenarioState)
		branchToShip := gitdomain.LocalBranchNameOrPanic(stringss.TrimSpace(branchName))
	...
	sc.Step(`^the coworker is on the "([^"]*)" branch$`, func(ctx context.Context, branch string) {
		state := ctx.Value(keyScenarioState).(*ScenarioState)
		state.fixture.CoworkerRepo.GetOrPanic().CheckoutBranch(gitdomain.LocalBranchNameOrPanic(stringss.TrimSpace(branch)))
	})
	...
	sc.Step(`^the coworker sets the parent branch of "([^"]*)" as "([^"]*)"$`, func(ctx context.Context, childBranch, parentBranch string) {
		state := ctx.Value(keyScenarioState).(*ScenarioState)
		coworkerRepo := state.fixture.CoworkerRepo.GetOrPanic()
		_ = coworkerRepo.Config.NormalConfig.SetParent(coworkerRepo.TestRunner, gitdomain.LocalBranchNameOrPanic(stringss.TrimSpace(childBranch)), gitdomain.LocalBranchNameOrPanic(stringss.TrimSpace(parentBranch)))
	})

	sc.Step(`^the coworker sets the "sync-feature-strategy" to "(merge|rebase)"$`, func(ctx context.Context, value string) {
		state := ctx.Value(keyScenarioState).(*ScenarioState)
		coworkerRepo := state.fixture.CoworkerRepo.GetOrPanic()
		syncFeatureStrategy := asserts.NoError1(configdomain.ParseSyncFeatureStrategy(stringss.TrimSpace(value), "test"))
	...
	sc.Step(`^the current branch in the other worktree is (?:now|still) "([^"]*)"$`, func(ctx context.Context, expected string) error {
		...
		if !actual.EqualSome(gitdomain.LocalBranchNameOrPanic(stringss.TrimSpace(expected))) {
	...
	sc.Step(`^the current branch is "([^"]*)"$`, func(ctx context.Context, name string) error {
		state := ctx.Value(keyScenarioState).(*ScenarioState)
		devRepo := state.fixture.DevRepo.GetOrPanic()
		branch := gitdomain.LocalBranchNameOrPanic(stringss.TrimSpace(name))
	...
	sc.Step(`^the current branch is "([^"]*)" and the previous branch is "([^"]*)"$`, func(ctx context.Context, currentText, previousText string) {
		state := ctx.Value(keyScenarioState).(*ScenarioState)
		devRepo := state.fixture.DevRepo.GetOrPanic()
		current := gitdomain.LocalBranchNameOrPanic(stringss.TrimSpace(currentText))
		previous := gitdomain.LocalBranchNameOrPanic(stringss.TrimSpace(previousText))
	...
	sc.Step(`^the main branch is "([^"]+)"$`, func(ctx context.Context, name string) error {
		...
		repo := state.fixture.DevRepo.GetOrPanic()
		devRepo := state.fixture.DevRepo.GetOrPanic()
		return devRepo.Config.SetMainBranch(gitdomain.LocalBranchNameOrPanic(stringss.TrimSpace(name)), repo.TestRunner)
	})
	...
	sc.Step(`^the main branch is (?:now|still) "([^"]*)"$`, func(ctx context.Context, want string) error {
		...
		if !have.EqualSome(gitdomain.LocalBranchNameOrPanic(stringss.TrimSpace(want))) {
