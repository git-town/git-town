...
func parsePullRequest(pullRequest *github.PullRequest) forgedomain.ProposalData {
	return forgedomain.ProposalData{
		...
		Source:       gitdomain.LocalBranchNameOrPanic(stringss.Trimmed(pullRequest.Head.GetRef())), // we can assume the GitHub API provides correct strings
		Target:       gitdomain.LocalBranchNameOrPanic(stringss.Trimmed(pullRequest.Base.GetRef())), // we can assume the GitHub API provides correct strings
