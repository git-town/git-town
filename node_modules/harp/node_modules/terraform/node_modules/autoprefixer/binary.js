...
var Binary = (function () {
    ...
    Binary.prototype.info = function info(done) {
        this.print((0, _autoprefixerCore2['default'])(this.pluginOptions).info());
        done();
    };
    ...
    Binary.prototype.compiler = function compiler() {
        if (!this.compilerCache) {
            this.compilerCache = (0, _postcss2['default'])([(0, _autoprefixerCore2['default'])(this.pluginOptions)]);
        }
    ...
    Binary.prototype.compileCSS = function compileCSS(css, output, input) {
        ...
        this.compiler().process(css, opts)['catch'](function (error) {
            if (error.indexOf && error.indexOf('Unknown browser') !== -1) {
                _this.error('autoprefixer: ' + error);
            } else if (error.autoprefixer) {
                _this.error('autoprefixer: ' + error.message);
            } else if (error.name === 'CssSyntaxError') {
                var text = error.message + error.showSourceCode();
                _this.error('autoprefixer:' + text);
            } else {
                _this.error('autoprefixer: Internal error');
                if (error.stack) {
                    _this.error('');
