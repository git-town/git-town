...
- Git Town now automatically waits and retries Git operations if it encounters concurrent access by another Git process. These problems happen for example when using IDEs that run Git commands in the background ([#3629](https://github.com/git-town/git-town/issues/3629)).
- Git Town now provides support for status indicators in your shell prompt. If a Git Town command got interrupted by a merge conflict, you can now add the name of the pending Git Town command to your shell prompt. This helps you remember to run `git town continue` to finish the pending command ([#2208](https://github.com/git-town/git-town/issues/2208)).
- When you run `git town propose` and a proposal for the branch to propose already exists, you are now taken directly to the website of that proposal ([#2362](https://github.com/git-town/git-town/issues/2362)).
- Git Town now prints a status indicator in the CLI output when it makes API requests. This helps you see better what Git Town is currently doing and how slowness is coming from ([#3892](https://github.com/git-town/git-town/pull/3892)).
...
- If you have a merge conflict between your uncommitted changes and branch ancestry, Git Town commands will fail when running `git stash pop` at the end. Previously when running `git town continue` it tried to pop the stash again, causing the same merge conflict to happen again. Now Git Town assumes that you have resolved the merge conflicts when running `git town continue` and deletes the stash entry instead. To re-apply the conflicting stash entry manually run `git stash pop` manually before running `git town continue`. This prevents outdated entries on the Git stash ([#3886](https://github.com/git-town/git-town/pull/3886)).
- `git town continue` now re-runs all failed Git operations. This helps Git Town recover from a wider range of unexpected problems ([#3887](https://github.com/git-town/git-town/pull/3887), [#3885](https://github.com/git-town/git-town/pull/3885)).
