...
#### Bug Fixes
* `git town-sync --all`: don't prompt for parent of perennial branches
  ([#727](https://github.com/Originate/git-town/issues/727))

## 0.10.1 (2016-06-23)

#### New Features
* `git town-hack`: add configuration for whether or not to push
  ([#720](https://github.com/Originate/git-town/issues/720))

#### Bug Fixes
* configuration: make branch order consistent
* `git town-ship`: update uncommitted changes error message
  ([#718](https://github.com/Originate/git-town/issues/718))

## 0.10.0 (2016-01-21)

#### BREAKING CHANGES
* `git town-prune-branches`: new functionality - delete branches whose tracking branch no longer exists
  ([#702](https://github.com/Originate/git-town/issues/702))

#### New Features
* `git town-sync`: add configuration option to merge the main branch / perennial branches with their upstream
  ([#671](https://github.com/Originate/git-town/issues/671))
* `git town-hack`, `git town-ship`: support for running in subfolders

#### Bug Fixes
* internationalize check for undefined function
  ([#678](https://github.com/Originate/git-town/issues/678))
* `git town-new-pull-request`: ability to continue after conflicts
  ([#700](https://github.com/Originate/git-town/issues/700))

## 0.9.0 (2015-10-17)

#### BREAKING CHANGES
* remove `git town-sync-fork`

#### New Features
* `git town-new-pull-request`: support forked repos
  ([#682](https://github.com/Originate/git-town/issues/682))
* `git town-sync`: if there is a remote upstream, the main branch is synced with its upstream counterpart
...
* `git town-hack`: pushes the new branch to the remote repository
  ([#664](https://github.com/Originate/git-town/issues/664))
* `git town-new-pull-request`: syncs the branch before creating the pull request
  ([#367](https://github.com/Originate/git-town/issues/367))
* `git town-sync --all`: pushes tags
...
* `git town-kill`: remote only branch
  ([#380](https://github.com/Originate/git-town/issues/380))
* `git town-ship`: prompt when there are multiple authors
  ([#486](https://github.com/Originate/git-town/issues/486))

## 0.7.2 (2015-08-28)

* `git town-sync --all`: fix parent branch prompt
* `git town-ship`: comment out default commit message
  ([#382](https://github.com/Originate/git-town/issues/382))

## 0.7.1 (2015-08-27)

* `git town-ship`: add ability to ship remote only branch
  ([#593](https://github.com/Originate/git-town/issues/593))
* `git town-new-pull-request`/`git town-repo`: remove empty line output
  ([#602](https://github.com/Originate/git-town/issues/602))
* `git town-kill`: prompt for unknown parent branch
  ([#603](https://github.com/Originate/git-town/issues/603))
* `git town-sync --all`: prompt for unknown parent branch
...
* add `git town-rename-branch`
  ([#474](https://github.com/Originate/git-town/issues/474))
* rename `git pull-request` to `git town-new-pull-request`
...
* support for working without a remote repository for **git extract**, **git town-hack**, **git town-kill**, **git town-ship**, and **git town-sync**
...
* improved error messages when run outside a git town-repository
* improved setup wizard for initial configuration in a git town-repository
...
* **git extract:**
  ...
  * added restriction: if no commits are provided, errors if the current branch does not have any have extractable commits (commits not in the main branch)
    ([#269](https://github.com/Originate/git-town/issues/269))
* **git town-hack:** errors if branch exists remotely
...
* **git town-sync-fork:** no longer automatically sets upstream configuration
...
* **git town-kill:** completely removes a feature branch
...
* **git town-sync:** pushes tags to the remote when running on the main branch
...
* **git town-ship:**
  ...
  * git town-ship takes an optional branch name
...
* much faster testing thanks to completely local test git town-repos
...
## 0.1.0 (2014-05-22)
* git town-hack, git town-sync, git extract, git town-ship
